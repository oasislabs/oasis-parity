...
use ethstore::ethkey::{verify_address, Generator, KeyPair, Random, Secret};
use ethstore::{EthStore, SecretVaultRef, SimpleSecretStore, StoreAccountRef};
...
fn secret_store_create_account() {
	...
	assert!(store
		.insert_account(SecretVaultRef::Root, random_secret(), "")
		.is_ok());
