...
impl SenderReservations {
	...
	pub fn reserve_nonce(&mut self, minimal: U256) -> Reserved {
		...
		let prospective_value = cmp::max(minimal, self.prospective_value - U256::from(dropped));
		self.prospective_value = prospective_value + U256::from(1);
...
impl Ready {
	...
	pub fn mark_used(mut self) {
		...
		next.send(self.value + U256::from(1)).expect(Self::RECV_PROOF);
	}
}
