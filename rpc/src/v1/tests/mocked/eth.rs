...
use ethcore::client::{
	BlockChainClient, BlockId, EachBlockWith, Executed, TestBlockChainClient, TransactionId,
};
...
use v1::tests::helpers::{Config, TestMinerService, TestSnapshotService, TestSyncProvider};
use v1::types::Origin;
use v1::{
	Eth, EthClient, EthClientOptions, EthFilter, EthFilterClient, EthSigning, SigningUnsafeClient,
};
...
fn rpc_eth_sign() {
	...
	let account = tester
		.accounts_provider
		.insert_account(Secret::from([69u8; 32]), "abcd")
		.unwrap();
	tester
		.accounts_provider
...
fn rpc_eth_author() {
	...
	assert_eq!(
		tester.io.handle_request_sync(req),
		Some(make_res(Address::zero()))
	);
...
fn rpc_eth_accounts() {
	...
	tester
		.accounts_provider
		.set_new_dapps_addresses(Some(vec![1.into()]))
		.unwrap();
	// even with some account it should return empty list (no dapp detected)
	let request = r#"{"jsonrpc": "2.0", "method": "eth_accounts", "params": [], "id": 1}"#;
	let response =
		r#"{"jsonrpc":"2.0","result":["0x0000000000000000000000000000000000000001"],"id":1}"#;
	assert_eq!(
		tester.io.handle_request_sync(request),
		Some(response.to_owned())
	);
	...
	tester
		.accounts_provider
		.set_dapp_addresses("app1".into(), Some(vec![10.into()]))
		.unwrap();
	let request = r#"{"jsonrpc": "2.0", "method": "eth_accounts", "params": [], "id": 1}"#;
	let response =
		r#"{"jsonrpc":"2.0","result":["0x000000000000000000000000000000000000000a"],"id":1}"#;
	let mut meta = Metadata::default();
	meta.origin = Origin::Dapps("app1".into());
	assert_eq!(
		(*tester.io).handle_request_sync(request, meta),
		Some(response.to_owned())
	);
}
