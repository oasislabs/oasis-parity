...
fn rpc_parity_set_and_get_dapps_accounts() {
	...
	assert_eq!(
		tester.accounts.dapp_addresses("app1".into()).unwrap(),
		vec![]
	);
	...
	let request =
		r#"{"jsonrpc": "2.0", "method": "parity_getDappAddresses","params":["app1"], "id": 1}"#;
	let response =
...
fn rpc_parity_set_and_get_new_dapps_whitelist() {
	...
	// then
	assert_eq!(
		tester.accounts.new_dapps_addresses().unwrap(),
		Some(vec![10.into()])
	);
	let request =
		r#"{"jsonrpc": "2.0", "method": "parity_getNewDappsAddresses","params":[], "id": 1}"#;
	let response =
		r#"{"jsonrpc":"2.0","result":["0x000000000000000000000000000000000000000a"],"id":1}"#;
	assert_eq!(
		tester.io.handle_request_sync(request),
	...
	// when set to empty
	let request =
		r#"{"jsonrpc": "2.0", "method": "parity_setNewDappsAddresses","params":[null], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
	assert_eq!(
		tester.io.handle_request_sync(request),
		Some(response.to_owned())
...
fn rpc_parity_set_and_get_new_dapps_default_address() {
	...
	tester.accounts.set_address_name(10.into(), "10".into());
	assert_eq!(
		tester.accounts.new_dapps_default_address().unwrap(),
		0.into()
	);
	...
	let request =
		r#"{"jsonrpc": "2.0", "method": "parity_getNewDappsDefaultAddress","params":[], "id": 1}"#;
	let response =
		r#"{"jsonrpc":"2.0","result":"0x000000000000000000000000000000000000000a","id":1}"#;
	assert_eq!(
		tester.io.handle_request_sync(request),
...
fn rpc_parity_change_vault() {
	...
	let (address, _) = tester
		.accounts
		.new_account_and_public("root_password")
		.unwrap();
	assert!(tester.accounts.create_vault("vault1", "password1").is_ok());
...
fn rpc_parity_vault_adds_vault_field_to_acount_meta() {
	...
	let (address1, _) = tester
		.accounts
		.new_account_and_public("root_password1")
		.unwrap();
	let uuid1 = tester
		.accounts
...
fn should_export_account() {
	...
	tester
		.accounts
		.import_wallet(wallet.as_bytes(), "parity-export-test", false)
		.unwrap();
	let accounts = tester.accounts.accounts().unwrap();
	assert_eq!(accounts.len(), 1);
