	...
	Rr: Fn() + 'static + Send,
{
	...
	match command.cmd {
		...
		Cmd::SignerSign {
			...
			port,
			authfile,
		} => rpc_cli::signer_sign(id, pwfile, port, authfile)
			.map(|s| ExecutionAction::Instant(Some(s))),
		Cmd::SignerList { port, authfile } => {
			rpc_cli::signer_list(port, authfile).map(|s| ExecutionAction::Instant(Some(s)))
		}
		Cmd::SignerReject { id, port, authfile } => {
			rpc_cli::signer_reject(id, port, authfile).map(|s| ExecutionAction::Instant(Some(s)))
		}
...
fn open_dapp(
	...
	rpc_conf: &rpc::HttpConfiguration,
	dapp: &str,
) -> Result<(), String> {
