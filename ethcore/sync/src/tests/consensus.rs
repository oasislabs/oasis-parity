...
fn authority_round() {
	...
	let mut net =
		TestNet::with_spec_and_accounts(2, SyncConfig::default(), Spec::new_test_round, Some(ap));
...
fn tendermint() {
	...
	let mut net = TestNet::with_spec_and_accounts(
		2,
		SyncConfig::default(),
	...
	net.peer(1)
		.miner
		.set_author(s1.address(), Some("".into()))
		.unwrap();
