...
pub use private_transactions::{
	PrivateTransactionDesc, PrivateTransactionSigningDesc, SigningStore, VerificationStore,
};

use bytes::Bytes;
use ethcore::account_provider::AccountProvider;
use ethcore::client::{
	BlockId, CallContract, ChainMessageType, ChainNotify, ChainRoute, Client, ClientIoMessage,
};
...
use transaction::{Action, SignedTransaction, Transaction, UnverifiedTransaction};
...
impl Provider where {
	...
	pub fn create_private_transaction(
		...
		trace!(
			"Creating private transaction from regular transaction: {:?}",
			signed_transaction
		);
		...
		match signed_transaction.action {
			...
			Action::Call(contract) => {
				...
				trace!(
					"Private transaction created, encrypted transaction: {:?}, private state: {:?}",
					private,
					private_state
				);
				...
				trace!(
					"Hashed effective private state for sender: {:?}",
					private_state_hash
				);
				self.transactions_for_signing.lock().add_transaction(
					private.hash(),
					signed_transaction,
					contract_validators,
					private_state,
	...
	fn extract_original_transaction(
		&self,
	...
	fn pool_client<'a>(
		&'a self,
		nonce_cache: &'a RwLock<HashMap<Address, U256>>,
	...
	fn process_queue(&self) -> Result<(), Error> {
		...
		let ready_transactions =
			verification_queue.ready_transactions(self.pool_client(&nonce_cache));
		for transaction in ready_transactions {
			...
			match verification_queue.private_transaction_descriptor(&transaction_hash) {
				Ok(desc) => {
					...
					if let Action::Call(contract) = transaction.signed().action {
						...
						let private_state = self
							.execute_private_transaction(BlockId::Latest, transaction.signed())?;
						let private_state_hash =
							self.calculate_state_hash(&private_state, contract_nonce);
						trace!(
							"Hashed effective private state for validator: {:?}",
							private_state_hash
						);
						let password =
							find_account_password(&self.passwords, &*self.accounts, &account);
						let signed_state =
							self.accounts.sign(account, password, private_state_hash)?;
						let signed_private_transaction =
							SignedPrivateTransaction::new(desc.private_hash, signed_state, None);
						trace!(
							"Sending signature for private transaction: {:?}",
							signed_private_transaction
						);
						self.broadcast_signed_private_transaction(
							signed_private_transaction.rlp_bytes().into_vec(),
						);
					} else {
				...
				Err(e) => {
					warn!(
						"Cannot retrieve descriptor for transaction with error {:?}",
						e
					);
				}
			}
	...
	fn last_required_signature(
		...
		match recover(&sign, &state_hash) {
			...
			Err(err) => {
				trace!(
					"Sender's state doesn't correspond to validator's, error {:?}",
					err
				);
				bail!(err);
			}
	...
	fn broadcast_private_transaction(&self, message: Bytes) {
		self.notify(|notify| {
			notify.broadcast(ChainMessageType::PrivateTransaction(message.clone()))
		});
	}
	...
	fn broadcast_signed_private_transaction(&self, message: Bytes) {
		self.notify(|notify| {
			notify.broadcast(ChainMessageType::SignedPrivateTransaction(message.clone()))
		});
	}
	...
	fn decrypt(&self, contract_address: &Address, data: &[u8]) -> Result<Bytes, Error> {
		...
		Ok(self
			.encryptor
			.decrypt(contract_address, &*self.accounts, data)?)
	}
		...
		X: ExtTracer,
	{
		...
		let contract_address = match transaction.action {
			Action::Call(ref contract_address) => {
				...
				trace!(
					"Patching contract at {:?}, code: {:?}, state: {:?}",
					contract_address,
					contract_code,
					contract_state
				);
		...
		let result = Executive::new(&mut state, &env_info, engine.machine())
		...
		let (encrypted_code, encrypted_storage) = match contract_address {
			...
			Some(address) => {
				...
				let enc_code = match code {
					Some(c) => {
						Some(self.encrypt(&address, &Self::iv_from_address(&address), &c)?)
					}
					None => None,
				};
				(
		...
		trace!(
			"Private contract executed. code: {:?}, state: {:?}, result: {:?}",
			encrypted_code,
			encrypted_storage,
			result.output
		);
	...
	pub fn public_creation_transaction(
		...
		validators: &[Address],
		gas_price: U256,
	) -> Result<(Transaction, Option<Address>), Error> {
		...
		Ok((
			Transaction {
				...
				gas_price: gas_price,
				value: source.value,
				data: Self::generate_constructor(
					validators,
					executed.code.unwrap_or_default(),
					executed.state,
				),
			},
...
impl Importer for Arc<Provider> {
	fn import_private_transaction(&self, rlp: &[u8]) -> Result<(), Error> {
		...
		match validation_account {
			...
			Some(&validation_account) => {
				...
				trace!(
					"The following account will be used for verification: {:?}",
					validation_account
				);
				let nonce_cache = Default::default();
				...
				self.channel
					.send(ClientIoMessage::execute(move |_| {
	...
	fn import_signed_private_transaction(&self, rlp: &[u8]) -> Result<(), Error> {
		...
		if last {
			...
			trace!(
				"Last required signature received, public transaction created: {:?}",
				public_tx
			);
			...
			let signer_account = self
				.signer_account
				.ok_or_else(|| ErrorKind::SignerAccountNotSet)?;
			...
			match self.transactions_for_signing.lock().remove(&private_hash) {
				...
				Err(err) => {
					trace!(
						"Failed to remove transaction from signing store, error: {:?}",
						err
					);
					bail!(err);
				}
			...
			match self
				...
				.lock()
				.add_signature(&private_hash, tx.signature())
			{
