	...
	pub fn populate_from_parent(&self, header: &mut Header, parent: &Header, gas_floor_target: U256, gas_ceil_target: U256) {
		...
		if let Some(ref ethash_params) = self.ethash_extensions {
			let gas_limit = {
				...
				let lower_limit = gas_limit - gas_limit / bound_divisor + U256::from(1);
				let upper_limit = gas_limit + gas_limit / bound_divisor - U256::from(1);
		...
		header.set_gas_limit({
			...
			if gas_limit < gas_floor_target {
				cmp::min(gas_floor_target, gas_limit + gas_limit / bound_divisor - U256::from(1))
			} else {
				cmp::max(gas_floor_target, gas_limit - gas_limit / bound_divisor + U256::from(1))
			}
		});
	}
