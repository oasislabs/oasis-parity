...
impl ExtTracer for NoopExtTracer {
    fn trace_storage_at(&mut self, _key: &H256) {
    }

    /// Transaction execution writes into persistent state.
    fn trace_set_storage(&mut self, _key: &H256) {
    }

    /// Transaction performs an exists query.
    fn trace_exists(&mut self, _address: &Address) {
    }

    /// Transactions performs an exists_and_not_null query.
    fn trace_exists_and_not_null(&mut self, _address: &Address) {
    }
    ...
    fn trace_balance(&mut self, _origin_address: &Address) {
    }
