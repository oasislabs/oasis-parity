		...
		C: BlockChain + CallContract + BlockProducer + Nonce + Sync,
	{
		...
		for tx in pending {
			...
			let result = client.verify_signed(&transaction)
				.map_err(|e| e.into())
				.and_then(|_| {
					open_block.push_transaction(transaction, None, &mut storage)
				});
...
impl miner::MinerService for Miner {
	...
	fn sensible_gas_price(&self) -> U256 {
		...
		self.transaction_queue.current_worst_gas_price() * 110u32 / U256::from(100)
	}

	fn sensible_gas_limit(&self) -> U256 {
		self.params.read().gas_range_target.0 / U256::from(5)
	}
