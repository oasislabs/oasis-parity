...
use super::{TrieError, TrieMut, Query};
...
use elastic_array::{ElasticArray1024, ElasticArray36};
use keccak::{KECCAK_NULL_RLP};
use bv::{BitSlice, BitVec, BlockType, BitSliceable};

type NodeKey = ElasticArray36<u8>;
type PartialKey<'a> = BitSlice<'a, u8>;
...
pub enum LookupNode<'a> {
	/// Null trie node; could be an empty root or an empty branch entry.
	Empty,
	/// Leaf node; has key slice and value. Value may not be empty.
	Leaf(PartialKey<'a>, &'a [u8]),
	/// Extension node; has key slice and node data. Data may not be null.
	Extension(PartialKey<'a>, &'a [u8]),
	/// Branch node; has array of 2 child nodes (each possibly null) and an optional immediate node data.
	Branch([&'a [u8]; 2], Option<&'a [u8]>)
}

impl<'a> LookupNode<'a> {
	/// Decode the `node_rlp` and return the Node.
	pub fn decoded(node_rlp: &'a [u8]) -> Result<Self, ::rlp::DecoderError> {
		...
		match r.prototype()? {
			...
			::rlp::Prototype::List(2) => match from_encoded_ext(r.at(0)?.data()?) {
				(slice, true) => Ok(LookupNode::Leaf(slice, r.at(1)?.data()?)),
				(slice, false) => Ok(LookupNode::Extension(slice, r.at(1)?.as_raw())),
			},
			// branch - first 2 are nodes, 3rd is a value (or empty).
			::rlp::Prototype::List(3) => {
				let mut nodes = [&[] as &[u8]; 2];
				for i in 0..2 {
					nodes[i] = r.at(i)?.as_raw();
				}
				Ok(LookupNode::Branch(nodes, if r.at(2)?.is_empty() { None } else { Some(r.at(2)?.data()?) }))
			},
			// an empty branch index.
			::rlp::Prototype::Data(0) => Ok(LookupNode::Empty),
	...
	pub fn encoded(&self) -> Vec<u8> {
		match *self {
			LookupNode::Leaf(ref slice, ref value) => {
				let mut stream = RlpStream::new_list(2);
				stream.append(&&encoded(&&*slice, true)[..]);
				stream.append(value);
				stream.out()
			},
			LookupNode::Extension(ref slice, ref raw_rlp) => {
				let mut stream = RlpStream::new_list(2);
				stream.append(&&encoded(&&*slice, false)[..]);
				stream.append_raw(raw_rlp, 1);
				stream.out()
			},
			LookupNode::Branch(ref nodes, ref value) => {
...
enum Node {
	...
	/// This key is encoded from a `BitSlice`, meaning it contains
	/// a flag indicating it is a leaf.
	Leaf(NodeKey, DBValue),
	/// An extension contains a shared portion of a key and a child node.
	/// The shared portion is encoded from a `BitSlice` meaning it contains
	/// a flag indicating it is an extension.
	/// The child node is always a branch.
	Extension(NodeKey, NodeHandle),
	/// A branch has up to 2 children and an optional value.
	Branch([Option<NodeHandle>; 2], Option<DBValue>)
}

impl Node {
	// load an inline node into memory or get the hash to do the lookup later.
	fn inline_or_hash(node: &[u8], db: &HashDB, storage: &mut NodeStorage) -> NodeHandle {
		LookupNode::try_decode_hash(&node)
			.map(NodeHandle::Hash)
			.unwrap_or_else(|| {
	...
	fn from_rlp(rlp: &[u8], db: &HashDB, storage: &mut NodeStorage) -> Self {
		match LookupNode::decoded(rlp).expect("rlp read from db; qed") {
			LookupNode::Empty => Node::Empty,
			LookupNode::Leaf(k, v) => Node::Leaf(encoded(&k, true), DBValue::from_slice(&v)),
			LookupNode::Extension(key, cb) => {
				Node::Extension(encoded(&key, false), Self::inline_or_hash(cb, db, storage))
			}
			LookupNode::Branch(ref children_rlp, val) => {
				let mut child = |i| {
...
fn common_prefix<Block: BlockType>(slice1: &BitSlice<Block>, slice2: &BitSlice<Block>) -> u64 {
	let mut prefix = 0u64;
...
fn encoded(slice: &BitSlice<u8>, is_leaf: bool) -> ElasticArray36<u8> {
	let prefix: u8 = (slice.len() % 8) as u8 | if is_leaf { 0x08u8 } else { 0x00u8 } ;
	let mut vec = ElasticArray36::new();
	vec.push(prefix);
	for b in slice.block_iter() {
		vec.push(b);
	}
	trace!(target: "trie", "encoded ({:?},{}) to {:?}", slice, is_leaf, vec);
	vec
}

fn from_encoded(bytes: &[u8]) -> BitSlice<u8> {
	let prefix = bytes[0];
	let last_bits = prefix & 0x07;
	let slice = BitSlice::from_slice(&bytes[1..]);
	let s = if last_bits != 0 {
		slice.bit_slice(.. (slice.len() as u64 - 8 + last_bits as u64))
	} else {
		slice
	};
	trace!(target: "trie", "decoded {:?} to {:?}", bytes, s);
	s
}

fn from_encoded_ext(bytes: &[u8]) -> (BitSlice<u8>, bool) {
	let prefix = bytes[0];
	let last_bits = prefix & 0x07;
	let is_leaf = prefix & 0x08 != 0;
	let slice = BitSlice::from_slice(&bytes[1..]);
	let s = if last_bits != 0 {
		(slice.bit_slice(.. (slice.len() as u64 - 8 + last_bits as u64)), is_leaf)
	} else {
		(slice, is_leaf)
	};
	trace!(target: "trie", "decoded_ext {:?} to {:?}", bytes, s);
	s
}
fn new_composed<Block: BlockType>(slice1: &BitSlice<Block>, slice2: &BitSlice<Block>) -> BitVec<Block> {
	let mut vec = BitVec::with_capacity(slice1.len() + slice2.len());
	for i in 0 .. slice1.len() { vec.push(slice1[i]); }
	for i in 0 .. slice2.len() { vec.push(slice2[i]); }
	vec
}

fn starts_with<Block: BlockType>(what: &BitSlice<Block>, with: &BitSlice<Block>) -> bool {
	if with.len() > what.len() {
		return false;
	}
	for i in 0 .. with.len() {
...
impl<'a, Q: Query> Lookup<'a, Q> {
	...
	pub fn look_up(mut self, mut key: PartialKey) -> super::Result<Option<Q::Item>> {
		...
		for depth in 0.. {
			...
			loop {
				match LookupNode::decoded(node_data).expect("rlp read from db; qed") {
					LookupNode::Leaf(slice, value) => {
						return Ok(match slice == key {
							true => Some(self.query.decode(value)),
							false => None,
						})
					}
					LookupNode::Extension(slice, item) => {
						if starts_with(&key, &slice) {
							node_data = item;
							key = key.bit_slice(slice.len()..);
						} else {
							return Ok(None)
						}
					}
					LookupNode::Branch(children, value) => match key.is_empty() {
						true => return Ok(value.map(move |val| self.query.decode(val))),
						false => {
							node_data = children[key[0] as usize];
							key = key.bit_slice(1..);
						}
					},
					_ => return Ok(None),
				...
				if let Some(h) = LookupNode::try_decode_hash(&node_data) {
					hash = h;
					break
				}
			}
...
impl<'a> TrieDBMut<'a> {
	...
	fn lookup<'x, 'key>(&'x self, mut partial: BitSlice<'key, u8>, handle: &NodeHandle) -> super::Result<Option<DBValue>>
		where 'x: 'key
	{
		let mut handle = handle;
		loop {
			trace!(target: "trie", "lookup loop partial={:?}", partial);
			let (mid, child) = match *handle {
				NodeHandle::Hash(ref hash) => {
					trace!(target: "trie", "lookup HASH={:?}", hash);
					return Lookup {
						db: &*self.db,
				...
				NodeHandle::InMemory(ref handle) => {
					match self.storage[handle] {
						...
						Node::Leaf(ref key, ref value) => {
							if from_encoded(key) == partial {
								trace!(target: "trie", "lookup LEAF matched={:?}", key);
								return Ok(Some(DBValue::from_slice(value)));
							} else {
								trace!(target: "trie", "lookup LEAF mismatch {:?} vs {:?}", key, partial);
								return Ok(None);
							}
						}
						Node::Extension(ref key, ref child) => {
							let slice = from_encoded(key);
							if starts_with(&partial, &slice) {
								(slice.len(), child)
							} else {
								return Ok(None);
							}
			...
			partial = partial.bit_slice(mid..);
			handle = child;
		}
	}

	/// insert a key, value pair into the trie, creating new nodes if necessary.
	fn insert_at(&mut self, handle: NodeHandle, partial: PartialKey, value: DBValue, old_val: &mut Option<DBValue>)
	...
	fn insert_inspector(&mut self, node: Node, partial: PartialKey, value: DBValue, old_val: &mut Option<DBValue>)
		-> super::Result<InsertAction>
	{
		...
		Ok(match node {
			Node::Empty => {
				...
				InsertAction::Replace(Node::Leaf(encoded(&partial, true), value))
			}
			Node::Branch(mut children, stored_value) => {
				...
				if partial.is_empty() {
					...
					*old_val = stored_value;
					...
					let partial = partial.bit_slice(1..);
					if let Some(child) = children[idx].take() {
						...
						let leaf = self.storage.alloc(Stored::New(Node::Leaf(encoded(&partial, true), value)));
			...
			Node::Leaf(e, stored_value) => {
				let existing_key = from_encoded(&e);
				let cp = common_prefix(&existing_key, &partial);
				if cp == existing_key.len() && cp == partial.len() {
					...
					*old_val = Some(stored_value);
					...
					match unchanged {
						// unchanged. restore
						true => InsertAction::Restore(Node::Leaf(e.clone(), value)),
						false => InsertAction::Replace(Node::Leaf(e.clone(), value)),
					}
				} else if cp == 0 {
					...
					let branch = if existing_key.is_empty() {
						...
						let new_leaf = Node::Leaf(encoded(&existing_key.bit_slice(1..), true), stored_value);
					...
					let branch = Node::Branch([None, None], Some(stored_value));
					// augment the new branch.
					let branch = self.insert_inspector(branch, partial.bit_slice(cp..), value, old_val)?.unwrap_node();

					// always replace since we took a leaf and made an extension.
					let branch_handle = self.storage.alloc(Stored::New(branch)).into();
					InsertAction::Replace(Node::Extension(encoded(&existing_key, false), branch_handle))
				} else {
					...
					let low = Node::Leaf(encoded(&existing_key.bit_slice(cp..), true), stored_value);

					// augment it. this will result in the Leaf -> cp == 0 routine,
					// which creates a branch.
					let augmented_low = self.insert_inspector(low, partial.bit_slice(cp..), value, old_val)?.unwrap_node();

					// make an extension using it. this is a replacement.
					InsertAction::Replace(Node::Extension(
						encoded(&existing_key.bit_slice(..cp), false),
						self.storage.alloc(Stored::New(augmented_low)).into()
					))
				}
			}
			Node::Extension(e, child_branch) => {
				let existing_key = from_encoded(&e);
				let cp = common_prefix(&partial, &existing_key);
				if cp == 0 {
					trace!(target: "trie", "no-common-prefix, not-both-empty (exist={:?}; new={:?}): TRANSMUTE,AUGMENT", existing_key.len(), partial.len());
					...
					children[idx] = if existing_key.len() == 1 {
						...
						let ext = Node::Extension(encoded(&existing_key.bit_slice(1..), false), child_branch);
					...
					let (new_child, changed) = self.insert_at(child_branch, partial.bit_slice(cp..), value, old_val)?;
					let new_ext = Node::Extension(encoded(&existing_key, false), new_child.into());
					...
					// partially-shared.
					let low = Node::Extension(encoded(&existing_key.bit_slice(cp..), false), child_branch);
	...
	fn remove_at(&mut self, handle: NodeHandle, partial: PartialKey, old_val: &mut Option<DBValue>)
	...
	fn remove_inspector(&mut self, node: Node, partial: PartialKey, old_val: &mut Option<DBValue>) -> super::Result<Action> {
		Ok(match (node, partial.is_empty()) {
			...
			(Node::Branch(mut children, value), false) => {
				...
				if let Some(child) = children[idx].take() {
					...
					match self.remove_at(child, partial.bit_slice(1..), old_val)? {
						Some((new, changed)) => {
			...
			(Node::Leaf(encoded, value), _) => {
				if from_encoded(&encoded) == partial {
					// this is the node we were looking for. Let's delete it.
					*old_val = Some(value);
					Action::Delete
				} else {
					// leaf the node alone.
					trace!(target: "trie", "restoring leaf wrong partial, partial={:?}, existing={:?}", partial, from_encoded(&encoded));
					Action::Restore(Node::Leaf(encoded, value))
				}
			}
			(Node::Extension(encoded, child_branch), _) => {
				let (cp, existing_len) = {
					let existing_key = from_encoded(&encoded);
					(common_prefix(&existing_key, &partial), existing_key.len())
				};
				if cp == existing_len {
					// try to remove from the child branch.
					trace!(target: "trie", "removing from extension child, partial={:?}", partial);
					match self.remove_at(child_branch, partial.bit_slice(cp..), old_val)? {
			...
			Node::Branch(mut children, value) => {
				...
				for i in 0..2 {
					match (children[i].is_none(), &used_index) {
						(false, &UsedIndex::None) => used_index = UsedIndex::One(i == 1),
						(false, &UsedIndex::One(_)) => {
							used_index = UsedIndex::Two;
							break;
						}
						_ => continue,
					}
				...
				match (used_index, value) {
					...
					(UsedIndex::One(a), None) => {
						...
						let new_partial = encoded(&bit_vec![a].as_slice(), false);
					...
					(UsedIndex::None, Some(value)) => {
						...
						Ok(Node::Leaf(Default::default(), value))
					}
					(_, value) => {
						// all is well.
			...
			Node::Extension(partial, child) => {
				...
				match child_node {
					Node::Extension(sub_partial, sub_child) => {
						...
						let partial = from_encoded(&partial);
						let sub_partial = from_encoded(&sub_partial);

						let new_partial = new_composed(&partial, &sub_partial);
						trace!(target: "trie", "fixing: extension combination. new_partial={:?}", new_partial);
						self.fix(Node::Extension(encoded(&new_partial.as_slice(), false), sub_child))
					}
					Node::Leaf(sub_partial, value) => {
						...
						Ok(Node::Leaf(encoded(&new_partial.as_slice(), true), value))
					}
					child_node => {
...
impl<'a> TrieMut for TrieDBMut<'a> {
	...
	fn get<'x, 'key>(&'x self, key: &'key [u8]) -> super::Result<Option<DBValue>> where 'x: 'key {
		self.lookup(BitSlice::from_slice(key), &self.root_handle)
	}
	...
	fn remove(&mut self, key: &[u8]) -> super::Result<Option<DBValue>> {
		...
		let root_handle = self.root_handle();
		let key = BitSlice::from_slice(key);
		let mut old_val = None;
...
mod tests {
	...
	fn test_at_one() {
		...
		let mut root = H256::new();
		let mut t = TrieDBMut::new(&mut memdb, &mut root);
		t.insert(&[0x01], &[0x01u8, 0x23]).unwrap();
		assert_eq!(t.get(&[0x1]).unwrap().unwrap(), DBValue::from_slice(&[0x1u8, 0x23]));
		t.insert(&[0x01u8, 0x23], &[0x01u8, 0x23]).unwrap();
