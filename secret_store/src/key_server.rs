	...
	pub fn new(
		...
		let handle = thread::Builder::new()
			...
			.spawn(move || {
				...
				tx.send(cluster_client.map_err(Into::into))
					.expect("Rx is blocking upper thread.");
		...
		let cluster = rx
			.recv()
			.map_err(|e| Error::Internal(format!("error initializing event loop: {}", e)))??;
...
pub mod tests {
	...
	fn make_key_servers(
		start_port: u16,
		num_nodes: usize,
	) -> (Vec<KeyServerImpl>, Vec<Arc<DummyKeyStorage>>) {
		...
		let key_servers: Vec<_> = configs
			...
			.map(|(i, cfg)| {
				KeyServerImpl::new(
					&cfg,
					Arc::new(MapKeyServerSet::new(key_servers_set.clone())),
		...
		loop {
			if key_servers
				.iter()
				.all(|ks| ks.cluster().cluster_state().connected.len() == num_nodes - 1)
			{
				break;
			}
