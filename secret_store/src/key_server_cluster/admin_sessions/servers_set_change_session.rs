...
use key_server_cluster::message::{
	...
	InitializeShareChangeSession, KeyVersionNegotiationMessage, Message, ServersSetChangeCompleted,
	ServersSetChangeConsensusMessage, ServersSetChangeDelegate, ServersSetChangeDelegateResponse,
	ServersSetChangeError, ServersSetChangeMessage, ServersSetChangeShareAddMessage,
	ShareChangeKeyVersionNegotiation, UnknownSessions, UnknownSessionsRequest,
};
use key_server_cluster::share_change_session::{
...
impl ClusterSession for SessionImpl {
	...
	fn on_session_error(&self, node: &NodeId, error: Error) {
		...
		warn!(
			"{}: servers set change session failed: {} on {}",
...
pub mod tests {
	...
	use key_server_cluster::generation_session::tests::{
		generate_nodes_ids, MessageLoop as GenerationMessageLoop, Node as GenerationNode,
	};
	...
	impl MessageLoop {
		pub fn new(
			...
			master_node_id: NodeId,
			original_key_pair: Option<KeyPair>,
			new_nodes_ids: BTreeSet<NodeId>,
			removed_nodes_ids: BTreeSet<NodeId>,
			isolated_nodes_ids: BTreeSet<NodeId>,
		) -> Self {
			...
			let mut all_nodes_set: BTreeSet<_> = gml
				.nodes
				.keys()
			...
			let old_nodes = gml.nodes.iter().map(|n| {
				create_node(
					meta.clone(),
					admin_public.clone(),
					all_nodes_set.clone(),
			...
			let nodes: BTreeMap<_, _> = old_nodes
				.chain(new_nodes)
			...
			let all_set_signature =
				sign(admin_key_pair.secret(), &ordered_nodes_hash(&all_nodes_set)).unwrap();
		...
		pub fn process_message(&mut self, msg: (NodeId, NodeId, Message)) -> Result<(), Error> {
			match {
				match msg.2 {
					Message::ServersSetChange(ref message) => {
	...
	pub fn generate_key(threshold: usize, nodes_ids: BTreeSet<NodeId>) -> GenerationMessageLoop {
		...
		gml.master()
			.initialize(
				...
				false,
				threshold,
				gml.nodes.keys().cloned().collect::<BTreeSet<_>>().into(),
			)
			.unwrap();
	...
	fn node_added_using_servers_set_change() {
		...
		let nodes_to_add: BTreeSet<_> = (0..1)
			.map(|_| Random.generate().unwrap().public().clone())
			.collect();
		...
		ml.nodes[&master_node_id]
			.session
			.initialize(
				ml.nodes.keys().cloned().collect(),
				ml.all_set_signature.clone(),
				ml.new_set_signature.clone(),
			)
			.unwrap();
		ml.run();
	...
	fn node_moved_using_servers_set_change() {
		...
		assert!(ml
			...
			.iter()
			.filter(|&(k, _)| nodes_to_remove.contains(k))
			.all(|(_, v)| v.key_storage.get(&SessionId::default()).unwrap().is_none()));
	...
	fn node_removed_using_servers_set_change() {
		...
		let nodes_to_remove: BTreeSet<_> = gml.nodes.keys().cloned().skip(1).take(1).collect();
		let new_nodes_set: BTreeSet<_> = gml
			.nodes
			.keys()
			.cloned()
			.filter(|n| !nodes_to_remove.contains(&n))
	...
	fn isolated_node_removed_using_servers_set_change() {
		...
		let new_nodes_set: BTreeSet<_> = gml
			...
			.keys()
			.cloned()
			.filter(|n| !nodes_to_isolate.contains(&n))
			.collect();
		let mut ml = MessageLoop::new(
			&gml,
			master_node_id,
		...
		assert!(ml
			...
			.iter()
			.filter(|&(k, _)| nodes_to_isolate.contains(k))
			.all(|(_, v)| v.key_storage.get(&SessionId::default()).unwrap().is_some()));
	...
	fn having_less_than_required_nodes_after_change_does_not_fail_change_session() {
		...
		let mut ml = MessageLoop::new(
			&gml,
			master_node_id,
			Some(ml.original_key_pair.clone()),
			nodes_to_add.clone(),
			BTreeSet::new(),
			nodes_to_remove.clone(),
		);
		...
		assert!(ml
			.nodes
			.iter()
			.filter(|&(k, _)| nodes_to_add.contains(k))
			.all(|(_, v)| v.key_storage.get(&SessionId::default()).unwrap().is_none()));
