...
use key_server_cluster::{ClusterClient, ClusterSession, ClusterSessionsListener};
...
use {Error, NodeKeyPair, ServerKeyId};
		...
		match &task {
			&ServiceTask::GenerateServerKey(origin, server_key_id, author, threshold) => {
				data.retry_data
					...
					.insert(server_key_id.clone());
				log_service_task_result(
					&task,
			...
			&ServiceTask::StoreDocumentKey(
				...
				data.retry_data
					...
					.insert((server_key_id.clone(), author.clone()));
				log_service_task_result(
					&task,
					data.self_key_pair.public(),
			...
			&ServiceTask::RetrieveShadowDocumentKeyCommon(origin, server_key_id, requester) => {
				data.retry_data
					.lock()
					.affected_document_keys
					.insert((server_key_id.clone(), requester.clone()));
				log_service_task_result(
					&task,
					data.self_key_pair.public(),
			...
			&ServiceTask::Retry => Self::retry_pending_requests(&data)
	...
	fn process_server_key_generation_result(
		...
		match result {
			...
			Err(ref error) => {
				...
				let _ = data
					.contract
					.publish_server_key_generation_error(&origin, server_key_id)
					.map_err(|error| {
						warn!(target: "secretstore", "{}: failed to publish GenerateServerKey({}) error: {}",
		...
		match data.key_storage.get(server_key_id) {
			...
			Err(ref error) => {
				...
				let _ = data
					...
					.publish_server_key_retrieval_error(&origin, server_key_id)
					.map_err(|error| {
						warn!(target: "secretstore", "{}: failed to publish RetrieveServerKey({}) error: {}",
						data.self_key_pair.public(), server_key_id, error)
					});
				Err(format!("{}", error))
			}
	...
	fn store_document_key(
		...
		match store_result {
			...
			Err(ref error) => {
				...
				let _ = data
					.contract
					.publish_document_key_store_error(&origin, server_key_id)
					.map_err(|error| {
						warn!(target: "secretstore", "{}: failed to publish StoreDocumentKey({}) error: {}",
						data.self_key_pair.public(), server_key_id, error)
					});
				Err(format!("{}", error))
			}
	...
	fn retrieve_document_key_common(
		...
		match retrieval_result {
			...
			Err(ref error) => {
				...
				let _ = data
					.contract
					.publish_document_key_retrieval_error(&origin, server_key_id, requester)
					.map_err(|error| {
						warn!(target: "secretstore", "{}: failed to publish RetrieveDocumentKey({}) error: {}",
						data.self_key_pair.public(), server_key_id, error)
					});
				Err(format!("{}", error))
			}
...
fn is_processed_by_this_key_server(
	...
	let this_server_index = match servers
		.keys()
		.enumerate()
		.find(|&(_, s)| s == self_key_pair.public())
	{
		Some((index, _)) => index,
		None => return false,
	};
...
mod tests {
	...
	use key_storage::{DocumentKeyShare, KeyStorage};
	...
	fn server_key_is_retrieved_when_processing_retrieve_server_key_task() {
		...
		let listener =
			make_service_contract_listener(Some(contract.clone()), None, Some(key_storage), None);
		ServiceContractListener::process_service_task(
			&listener.data,
