...
use bytes::Bytes;
use ethcore::client::{
	BlockChainClient, BlockId, CallContract, ChainNotify, ChainRoute, Client, RegistryInfo,
};
...
impl CachedContract {
	pub fn new(
		...
		let server_set = key_servers
			.into_iter()
			.map(|(p, addr)| {
				let addr = format!("{}:{}", addr.address, addr.port)
					.parse()
					.map_err(|err| {
	...
	fn read_from_registry_if_required(
		&mut self,
		client: &Client,
		enacted: Vec<H256>,
		retracted: Vec<H256>,
		...
		let new_contract_addr = get_confirmed_block_hash(&*client, REQUEST_CONFIRMATIONS_REQUIRED)
			.and_then(|block_hash| {
				client.registry_address(
					KEY_SERVER_SET_CONTRACT_REGISTRY_NAME.to_owned(),
		...
		let is_set_changed = self.contract_address.is_some()
			&& enacted.iter().chain(retracted.iter()).any(|block_hash| {
				!client
					.logs(Filter {
						...
						topics: vec![
							Some(vec![
								*ADDED_EVENT_NAME_HASH,
								*REMOVED_EVENT_NAME_HASH,
								*MIGRATION_STARTED_EVENT_NAME_HASH,
								*MIGRATION_COMPLETED_EVENT_NAME_HASH,
							]),
							None,
							None,
							None,
	...
	fn read_from_registry(&mut self, client: &Client, new_contract_address: Option<Address>) {
		...
		let (new_set, migration) = match self.auto_migrate_enabled {
			true => {
				...
				let migration_set =
					Self::read_key_server_set(MigrationKeyServerSubset::default(), &do_call);
				...
				let migration_id = match migration_set.is_empty() {
					false => self
						...
						.call(&do_call)
						.map_err(|err| {
							trace!(target: "secretstore", "Error {} reading migration id from contract", err);
							err
						})
						.ok(),
					true => None,
				...
				let migration_master = match migration_set.is_empty() {
					false => self
						.contract
						.functions()
						.get_migration_master()
						.call(&do_call)
						.map_err(|err| {
							trace!(target: "secretstore", "Error {} reading migration master from contract", err);
							err
						})
						.ok()
						.and_then(|address| {
				...
				let is_migration_confirmed = match migration_set.is_empty() {
							...
							|| migration_set.contains_key(self.self_key_pair.public()) =>
					{
						self.contract
							...
							.call(self.self_key_pair.address(), &do_call)
							.map_err(|err| {
								trace!(target: "secretstore", "Error {} reading migration confirmation from contract", err);
								err
							})
							.ok()
					}
		...
		F: Fn(Vec<u8>) -> Result<Vec<u8>, String>,
	{
		...
		let key_servers_list = subset
			.read_list(&do_call)
			.map_err(|err| {
				warn!(target: "secretstore_net", "error {} reading list of key servers from contract", err);
				err
			})
			.unwrap_or_default();
